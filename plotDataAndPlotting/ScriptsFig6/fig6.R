...
#Version: 1
#Includes 5-Z-OXO treated, Tubacin anf control plots
...
image_paths <- list.files(path = "/projects/b1042/GoyalLab/Keerthana/MoiseEtAl/extractedData/selectedImages/figure6/2024_03_08_acetyltub_kocells", pattern = "\\.png$", recursive = TRUE, full.names = TRUE)
...
df_main <- df_comb
write.csv(df_comb, "~/Keerthana/MoiseEtAl/plotData/Fig6_data_SM9.csv")
...
subsampled_data <- df_comb %>%
  ...
  ungroup() 
write.csv(subsampled_data, "~/Keerthana/MoiseEtAl/plotData/Fig6_subsampled_data_SM9.csv")
...
plot <- ggplot(subsampled_data, aes(x=Treatment, y=Eccentricity, color=Condition)) +
  ...
  theme_classic(base_size=10) +  # Smaller base size for all text elements
  theme(
    axis.line=element_line(colour='black', linewidth=1.5),
    axis.ticks=element_line(colour="black", linewidth=1.5),
    text=element_text(size=10, family="Helvetica"),  # Consistent text size for all textual elements
...
# Print the plot
print(plot)
ggsave(plot, file = paste0("../Plots/Eccentricity_Fig6_SM9.svg"), width = 13.7, height =13.7)
...
svglite(paste0("../Plots/RadialOrientation_Fig6_SM9.svg"), height = 20, width = 10)
par(mfrow=c(4,2))
...
for (i in unique(df_comb$Treatment)) {
  for (j in unique(df_comb$Condition)) {
    ...
    text(x=label_radius * cos(angles_rad), y=label_radius * sin(angles_rad), labels=labels, cex=1)
    ...
    save_path <- paste0("~/Keerthana/MoiseEtAl/plotData/Fig6_SM9_RadialPlot_",i,"_",j,".csv")
  ...
  mtext("SM9", side = 3, outer = TRUE, line = -3, cex = 1.5)
}
