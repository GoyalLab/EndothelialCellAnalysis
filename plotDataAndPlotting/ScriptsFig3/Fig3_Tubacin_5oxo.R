...
image_paths <- list.files(path = "/home/mzo5929/Keerthana/MoiseEtAl/extractedData/selectedImages/figure3/2024_03_31_20tl_acetylmods_20x/", pattern = "\\.png$", recursive = TRUE, full.names = TRUE)
...
df_main <- df_comb
write.csv(df_comb, "~/Keerthana/MoiseEtAl/plotData/Fig3_data_20TL.csv")
...
subsampled_data <- df_comb %>%
  ...
  ungroup() 
write.csv(subsampled_data, "~/Keerthana/MoiseEtAl/plotData/Fig3_subsampled_data_20TL.csv")
...
# Print the plot
print(plot)
ggsave(plot, file = paste0("../Plots/Eccentricity_Fig3_20TL.svg"), width = 13.7, height =13.7)
...
svglite(paste0("../Plots/RadialOrientation_Fig3_20TL.svg"), height = 20, width = 10)
par(mfrow=c(4,2))
...
for (i in unique(df_comb$Treatment)) {
  for (j in unique(df_comb$Condition)) {
    ...
    save_path <- paste0("~/Keerthana/MoiseEtAl/plotData/Fig3_20TL_RadialPlot_",i,"_",j,".csv")
