########################################################################################
#Description: Histograms and plotting eccentricity while affecting acetylation
#Author: Keerthana Arun
#Version: 1
#Modified by Keerthana Arun on 15 April 2024
########################################################################################
library(ggplot2)
library(dplyr)
library(plotrix)
...
image_paths <- list.files(path = "/home/mzo5929/Keerthana/MoiseEtAl/extractedData/selectedImages/figure5/2024_03_31_20tl_doubletub_20x/", pattern = "\\.png$", recursive = TRUE, full.names = TRUE)
...
df_main <- df_comb
write.csv(df_comb, "~/Keerthana/MoiseEtAl/plotData/Fig5_data_20TL_Tub.csv")
...
subsampled_data <- df_comb %>%
  ...
  slice_sample(n = 140) %>%  # Subsample up to 200 observations per group
  ungroup() 
write.csv(subsampled_data, "~/Keerthana/MoiseEtAl/plotData/Fig5_subsampled_data_20TL_Tub.csv")
...
# Print the plot
print(plot)
ggsave(plot, file = paste0("../Plots/Eccentricity_Fig5_20TL_Tub.svg"), width = 13.7, height =13.7)
...
svglite(paste0("../Plots/RadialOrientation_Fig5_20TL_Tub.svg"), height = 20, width = 10)
...
#                show.grid.labels=0, show.centroid = F,  radial.lim=1.00001, cex = 1) 
#     n <- min(c(100, sum(df_comb$Treatment==i&df_comb$Condition==j)))
#     subsetted_df <- sample_n(df_comb[df_comb$Treatment==i&df_comb$Condition==j,],n)
#     save_path <- paste0("~/Keerthana/MoiseEtAl/plotData/Fig5RadialPlot_",i,"_",j,".csv")
#     write_csv(subsetted_df, save_path)
#     add.line <- subsetted_df
...
#     
#   }
#   mtext(text = paste("20TL Tub" ), side = 3, outer = TRUE, line = -3, cex = 1.5)
#   
# }
...
for (i in unique(df_comb$Treatment)) {
  for (j in unique(df_comb$Condition)) {
    ...
    save_path <- paste0("~/Keerthana/MoiseEtAl/plotData/Fig5_20TL_RadialPlot_", i, "_", j, ".csv")
    write.csv(subsetted_df, save_path)
  ...
  mtext(text = "20TL Tub", side = 3, outer = TRUE, line = -3, cex = 1.5)
}
dev.off()
